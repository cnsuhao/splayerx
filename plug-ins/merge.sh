...
detect_subtitles(){
        ...
        $mplayer -sub-fuzziness 1 -frames 0 '$movie' 2>/dev/null | sed -n 's/^SUB: Added subtitle file ([0-9]*): \(.*\)$/\1/p'
...
$mplayer -dumpsrtsub -subcp utf8  -noautosub -really-quiet -frames 0 -sub "${sub[1]}" "$movie" 2>>"$mplayer_err" && echo "Done"
mv dumpsub.srt "${sub[1]}-temp"
echo "Converting the 2nd subtitle file (${sub[2]}) to a time-based format..."
$mplayer -dumpsrtsub -subcp utf8  -noautosub -really-quiet -frames 0 -sub "${sub[2]}" "$movie" 2>>"$mplayer_err" && echo "Done"
...
Style: lang1style,Arial,72,15724527,15724527,0,4144959,-1,0,1,3,1,2,30,30,30,0,0
Style: lang2style,Arial,64,15724527,15724527,0,4144959,-1,0,1,1,1,2,5,5,4,0,0
...
if [ ! -f "$mplayer" ]; then echo "Mplayer file ($mplayer) does not exist. Going back to shell.."; exit 1; fi
